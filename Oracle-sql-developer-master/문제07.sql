...
from tblAddressBook
    group by substr(email, instr(email, '@'))
        having round(avg(length(email)), 1) = (select 
    max(round(avg(length(email)), 1))
from tblAddressBook
    group by substr(email, instr(email, '@')));
            

-- tblAddressBook. 평균 나이가 가장 많은 출신(hometown)들이 가지고 있는 직업 중 가장 많은 직업은? O
...
select
    hometown
from tblAddressBook
    where age = (select round(avg(age)) from tblAddressBook)
        group by hometown
            having count(*) = (
select
    max(count(*))
...
-- tblAddressBook. 남자 평균 나이보다 나이가 많은 서울 태생 + 직업을 가지고 있는 사람들을 가져오시오. O

select  
    *
from tblAddressBook
    where age > (select round(avg(age)) from tblAddressBook where gender = 'm') and hometown = '서울' and job is not null;



-- tblAddressBook. gmail.com을 사용하는 사람들의 성별 > 세대별(10,20,30,40대) 인원수를 가져오시오. O
...
-- tblAddressBook.  동명이인이 여러명 있습니다. 이 중 가장 인원수가 많은 동명이인(모든 이도윤)의 명단을 가져오시오. O
...
-- tblAddressBook. 가장 사람이 많은 직업의(332명) 세대별 비율을 구하시오. O
--    [10대]       [20대]       [30대]       [40대]
--    8.7%        30.7%        28.3%        32.2%
select
    round(count(case
...
-- tblVideo, tblRent, tblMember. '뽀뽀할까요' 라는 비디오를 빌려간 회원의 이름은? O
...
select
    v.name,
    case
        when v.qty - r."c" is null then v.qty
        else v.qty - r."c"
    end as "보유수량"
from tblVideo v
    left outer join (
select 
    video, 
    count(*) as "c"
    from tblRent where retdate is null group by video) r
        on v.seq = r.video;
            

          
select video, count(*) from tblRent where retdate is null group by video;

                
-- tblVideo, tblRent, tblMember, tblGenre. 2007년 2월에 대여된 구매내역을 가져오시오. 회원명, 비디오명, 언제, 대여가격 O
...
-- tblVideo, tblRent, tblMember. 현재 반납을 안한 회원명과 비디오명, 대여날짜를 가져오시오. O
...
select * from employees e
    inner join (select distinct job_id from employees where salary >= 12000) f
...
-- employees, jobs. 직책(Job Title)이 Sales Manager인 사원들의 입사년도와 입사년도(hire_date)별 평균 급여를 가져오시오. 년도를 기준으로 오름차순 정렬. O
...
-- 도시명(city)과 평균연봉, 해당 도시의 사원수를 가져오시오. 단, 도시에 근 무하는 사원이 10명 이상인 곳은 제외하고 가져오시오. O
